...
public class DefaultActivitiEngine implements ActivitiEngine, Initializable
{
    ...
    {
        this.processEngine.getRepositoryService().createDeployment().name("myDeployment")
            .addClasspathResource("DemoProcess1.bpmn").addClasspathResource("DemoProcess2.bpmn")
            .addClasspathResource("DemoProcess3.bpmn").deploy();
    }
    ...
    {
        return this.processEngine.getRepositoryService().createProcessDefinitionQuery().orderByDeploymentId().desc()
            .list();
    }
